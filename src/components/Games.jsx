...
function CoinModal({ account, genBal, check, notify, onEnd, onClose }) {
  ...
  const play=async()=>{
    ...
    try {
      ...
      setResult(gr); onEnd('coin', gr?.result==='WIN'?'win':'lose'); setPhase('result')
...
function DiceModal({ account, genBal, check, notify, onEnd, onClose }) {
  ...
  const play=async()=>{
    ...
    try {
      ...
      setResult(g); onEnd('dice', won?'win':'lose'); setPhase('result')
...
function RpsModal({ account, genBal, check, notify, onEnd, onClose }) {
  ...
  const play=async()=>{
    ...
    try {
      ...
      setResult(gr||{}); onEnd('rps', gr?.result==='WIN'?'win':gr?.result==='TIE'?'tie':'lose'); setPhase('result')
